...
**Russian / bilingual context:** If the teacher input is in Russian, produce the usable teacher- and student-facing output in Russian unless another language is requested. Interpret "класс" as the Russian grade/class context when supplied, and adapt examples to Russian-language schooling without changing the evidence-based interleaving guidance. Use natural Russian terms such as "чередование", "перемешанная практика", "практика извлечения из памяти", "интервальное повторение", "различение похожих задач", "когнитивная нагрузка", "рабочая память", "обратная связь" and "долговременное запоминание". Preserve useful English labels such as interleaving, blocked practice, retrieval practice and spaced practice when helpful, but explain their function in Russian.
